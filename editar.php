...
// VERIFICA SI SE RECIBIO UN ID PARA EDITAR
if(isset($_GET["txtID"])){
    $txtID = (isset($_GET["txtID"])) ? $_GET["txtID"] : "";
    
    // BUSCA LOS DATOS ACTUALES DEL USUARIO EN LA BASE DE DATOS
    $sentencia = $conexion->prepare("SELECT * FROM usuario WHERE ID = :id");
    $sentencia->bindParam(":id", $txtID);
    $sentencia->execute();
    $registro = $sentencia->fetch(PDO::FETCH_LAZY);
    
    // SI EXISTE EL USUARIO GUARDA SUS DATOS EN VARIABLES
    if($registro){
        $Nick = $registro["Nick"];
...
// VERIFICA SI SE ENVIO EL FORMULARIO CON LOS CAMBIOS
if($_POST){
    ...
    // ASIGNA LOS VALORES NUEVOS A LA CONSULTA
    ...
    // REDIRECCIONA AL INICIO CON UN MENSAJE DE EXITO
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="IdRol" id="IdRol" class="form-select form-select-lg" required>
                                <option value="<?php echo $IdRol; ?>"></option>
